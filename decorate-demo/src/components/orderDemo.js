...
export const OrderDemo = () => {
    ...
    const addOrder = async () => {
        await addRef.current.validateFields()
            .then(value => {
                axios.post(`${HOST}:${PORT}/orders/add`, {user: value})
                    .then(result => {
            ...
            <div style={{padding: "30px 20px"}}>
                ...
                <Modal title={"修改订单信息"} visible={modifyOrders} okText={"修改"} cancelText={"取消"}
                    ...
                    <Form ref={modifyRef}>
                        ...
                        <Form.Item label="电话" name="phone" initialValue={order.phone}
                                   rules={[
                                       {
                                           required: true,
                        ...
                        <Form.Item label='留言' name='message' initialValue={order.message}
                                   rules={[{
                                       require: true,
                                       message: '请输入留言'
                                   }
                        ...
                        <Form.Item label='状态' name='status' initialValue={order.status}
                                   rules={[{
                                       required: true
                                   }]}
                        >
                ...
                <Modal title={"添加订单"} visible={addOrders} okText={"添加"} cancelText={"取消"} onCancel={handleAddCancel}
                    ...
                    <Form ref={addRef}>
                        ...
                        <Form.Item label='装饰类型' name='type' initialValue={"别墅"}
                                   rules={[{
                                       require: true,
                                       message: '请选择装饰类型'
                                   }]}
